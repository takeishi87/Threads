...
最新のニュース・トピックを探すとき、または記事のネタ元・裏取りをするときは、以下のサイトを優先的に確認する（WebSearch の `allowed_domains` 指定や WebFetch の対象として活用）。

| サイト | URL | 主な用途 |
|--------|-----|---------|
| 法務省（入管庁含む） | https://www.moj.go.jp/ | 在留資格・入管法改正・制度運用の一次情報（最優先） |
| PR TIMES | https://prtimes.jp/ | 企業のプレスリリース・新サービス・調査データ |
| Yahoo!ニュース | https://news.yahoo.co.jp/ | 一般ニュースの横断チェック |
| 産経ニュース | https://www.sankei.com/ | 社会・政治・経済ニュース |
| ニコニコニュース | https://news.nicovideo.jp/?news_ref | 話題・トレンドの把握 |
| NBC（長崎放送）ニュース | https://www.nbc.or.jp/category/news/ | 地方・地域ニュース |
| 建設通信新聞デジタル | https://digital.kentsu.co.jp/ | 建設業界の専門ニュース（建設分野の記事に有用） |

> ⚠️ ニュースサイトの情報は二次情報のため、制度・数値に関わる内容は必ず省庁公式サイト（法務省・厚労省等）の一次情報で裏取りすること。WebFetch が 403 等で取得できない場合は WebSearch で該当ドメインを `allowed_domains` に指定して要約を取得する。
...
### デザイン方針：企業向けホワイトペーパー風

目指す印象は「行政資料ほど堅くない／営業資料ほど売り込みすぎない／コンサル資料のように整理されている／初めて読む人でも理解しやすい／専門会社として信頼できる」。STEP 3のスタイルルール（口語・体験談ベース）と矛盾しない範囲で、以下を各章・各セクションに適用する。

**セクション構成の考え方（各章の中で意識する）：**

1. 導入部分 — 読者の悩みを短く提示し、読むメリットを明確にする
2. 結論部分 — 先に答えを示し、読者を迷わせない（「結論：〇〇です。」）
3. 解説部分 — 専門知識をやさしく説明する
4. 比較部分 — 表で違いを整理する
5. 実務部分 — チェックリストやSTEPで行動に落とし込む
6. CTA部分 — 相談への心理的ハードルを下げる

**余白・読みやすさのルール：**

- 1段落を長くしない（3〜4行程度で改行）
- 見出しごとに話題を1つに絞る
- 表の前後には必ず説明文を入れる（表だけを置かない）
- 囲み枠（背景色つきボックス等）は1セクションに多くても1つまで
- 太字は使いすぎない（強調は本当に重要な箇所のみ）
- 読み手の視線が止まる場所（小見出し・リスト・表）を意識して配置する
...
- メリットだけでなく注意点も必ず書く
- 制度面は断定しすぎない（「〜となる可能性があります」等、確定情報のみ断定形を使う）
- 確認が必要な箇所には「要確認」と明記する
- 専門家への確認が必要な箇所には「専門家確認推奨」と明記する
- 株式会社Threadsが支援できる範囲（できること／できないこと）を明確に書く
...
- [ ] 各章に結論先出し・比較表・実務チェックリストのいずれかが入っている（ホワイトペーパー風構成）
- [ ] 注意点・「要確認」「専門家確認推奨」の記載がある
- [ ] 囲み枠が1セクションに2つ以上重なっていない
...
- ホワイトペーパー風デザイン方針の遵守（結論先出し・比較表・実務チェックリスト・要確認/専門家確認推奨の記載・囲み枠の過多がないか）
